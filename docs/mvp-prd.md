...
- 購入前プレビューは安心材料を提供するが、pack を自作できるほどの情報は出さない

## Discovery Policy

MVP の discovery は販売方式ではなく推薦方式として扱う。

- ユーザーは無料でランダムに 1 pack を表示できる
- discovery 自体に課金しない
- discovery の結果から固定価格の商品詳細へ遷移する
- discovery は「有料ガチャ」ではなく「ランダム推薦」として説明する
- 当たり、ハズレ、排出率、コンプ要素を前提にしない
- TUI 上ではガチャガチャ風の演出を用いてよいが、実態は無料の推薦体験であること
- 演出はレバー、カプセル排出、結果カード出現などの記号化された表現に留めること
...
1. ユーザーが TUI ストアを開く
2. ユーザーが pack を閲覧するか、無料のランダム discovery を実行する
...
- discovery の表現は推薦または発見であり、抽選課金と誤認されないこと
- discovery 画面はガチャガチャを想起させるレトロゲーム風の演出を持ってよいこと
- discovery 演出は短時間で完了し、結果画面から詳細へ即遷移できること
...
- 購入前に表示する preview は、pack の価値と安全性を判断できる範囲に留めること
- 購入前に prompt 全文、hook 全文、template 完全版、install script の詳細実装を表示しないこと
- 同梱物は全文ではなく、カテゴリ、件数、要約を中心に見せること
...
- ユーザーが購入前に安心して判断できるが、全文コピーはできない情報設計になっていること
